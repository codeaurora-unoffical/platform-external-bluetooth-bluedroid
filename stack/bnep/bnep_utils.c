...
UINT8 *bnep_process_control_packet (tBNEP_CONN *p_bcb, UINT8 *p, UINT16 *rem_len, BOOLEAN is_ext)
{
    ...
    *rem_len = *rem_len - 1;
    ...
    case BNEP_SETUP_CONNECTION_REQUEST_MSG:
        ...
        if (*rem_len < ((2 * len) + 1))
        {
            BNEP_TRACE_ERROR("BNEP Received BNEP_SETUP_CONNECTION_REQUEST_MSG with bad length");
            goto bad_packet_length;
        }
        if (!is_ext)
    ...
    case BNEP_FILTER_NET_TYPE_SET_MSG:
        ...
        if (*rem_len < (len + 2))
        {
            BNEP_TRACE_ERROR("BNEP Received BNEP_FILTER_NET_TYPE_SET_MSG with bad length");
            goto bad_packet_length;
        }
    ...
    case BNEP_FILTER_MULTI_ADDR_SET_MSG:
        ...
        if (*rem_len < (len + 2))
        {
            BNEP_TRACE_ERROR ("BNEP Received BNEP_FILTER_MULTI_ADDR_SET_MSG with bad length");
            goto bad_packet_length;
        }
