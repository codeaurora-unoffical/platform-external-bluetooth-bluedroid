...
static void btif_hf_upstreams_evt(UINT16 event, char* p_param)
{
    ...
    {
        BTIF_TRACE_ERROR2("%s: Invalid index %d", __FUNCTION__, idx);
        return;
    }
    ...
    if (call_setup_state != btif_hf_cb[idx].call_setup_state)
    {
        ...
        switch (call_setup_state)
        {
            ...
            case BTHF_CALL_STATE_DIALING:
                ...
                {
                    BTIF_TRACE_DEBUG1("%s: Already in call, don't set audio handle", __FUNCTION__);
                }
            ...
            case BTHF_CALL_STATE_ALERTING:
                ...
                {
                    BTIF_TRACE_DEBUG1("%s: Already in call or prev call state was dialing,"
