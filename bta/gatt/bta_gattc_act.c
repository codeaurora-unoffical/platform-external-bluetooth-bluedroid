...
void bta_gattc_process_api_refresh(tBTA_GATTC_CB *p_cb, tBTA_GATTC_DATA * p_msg)
{
    ...
    {
        /* try to find a CLCB */
        APPL_TRACE_DEBUG3( "%s : connected = %d  num_clcb = %d", __FUNCTION__,
        ...
        if (p_srvc_cb->connected && p_srvc_cb->num_clcb != 0)
        {
            ...
            if (found)
            {
                APPL_TRACE_DEBUG1("%s : found service record in cache, ", __FUNCTION__);
                if (p_msg->hdr.layer_specific==BTA_GATTC_REFRESH_NO_DISCOVERY)
                {
                    APPL_TRACE_DEBUG1("%s : NO_DISCOVERY flag set "
                                        "not invoking discovery", __FUNCTION__ );
                }
                else
                {
                    APPL_TRACE_DEBUG1("%s : invoking discovery", __FUNCTION__ );
                    bta_gattc_sm_execute(p_clcb, BTA_GATTC_INT_DISCOVER_EVT, NULL);
                }
                return;
            }
        ...
        APPL_TRACE_DEBUG1( "%s : Clearing cache ", __FUNCTION__ );
