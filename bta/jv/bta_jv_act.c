...
tBTA_JV_L2C_CB* bta_jv_alloc_l2c_cb(UINT16 sock_handle)
{
    ...
    for(i=0; i < BTA_JV_MAX_L2C_CONN; i++)
    {
        if (bta_jv_cb.l2c_cb[i].in_use == FALSE)
        {
            ...
            APPL_TRACE_DEBUG2( "bta_jv_alloc_l2c_cb: sock_handle:%d, jv handle %d",
    ...
    if(p_l2c_cb == NULL)
    {
        APPL_TRACE_ERROR2( "bta_jv_alloc_l2c_cb: sock_handle:%d, ctrl block exceeds "
...
tBTA_JV_STATUS bta_jv_free_l2c_cb(tBTA_JV_L2C_CB *p_l2c_cb, BOOLEAN del_sec_rec)
{
    ...
    APPL_TRACE_DEBUG2("bta_jv_free_l2c_cb:  sock_handle:%d jv handle %d",
    ...
    if(del_sec_rec == TRUE)
    {
        APPL_TRACE_DEBUG2("bta_jv_free_l2c_cb: bta_jv_free_sec_id  sock_handle:%d jv handle  %d",
                                                    p_l2c_cb->sock_handle, p_l2c_cb->handle);
...
{
    APPL_TRACE_DEBUG0("bta_jv_l2cap_reg_cback ");
    if(p_data)
...
static void bta_jv_l2cap_client_cback(UINT16 sock_handle, UINT16 event)
{
    ...
    if( !p_l2c_cb || !p_l2c_cb->p_cback) {
        APPL_TRACE_ERROR0("bta_jv_l2cap_client_cback: cback not present ");
        return;
    }
    ...
    {
        APPL_TRACE_DEBUG1("bta_jv_l2cap_client_cback: L2C_SOCK_CONGESTED %d", event);
...
void bta_jv_l2cap_connect(tBTA_JV_MSG *p_data)
{
    ...
    {
        evt_data.status = BTA_JV_FAILURE;
        APPL_TRACE_ERROR2("sec_id:%d is zero or BTM_SetSecurityLevel failed, remote_scn:%d",
    ...
    if (p_l2c_cb != NULL)
    {
        switch(type)
        {
            case L2C_SOCK_DATA_CBACK_TYPE_INCOMING:
                APPL_TRACE_DEBUG1("bta_jv_l2c_data_co_cback, p_l2c_cb->p_pm_cb: %p",
                        p_l2c_cb->p_pm_cb);
...
static void bta_jv_l2cap_server_cback(UINT16 sock_handle, UINT16 event)
{
    ...
    if( !p_l2c_cb || !p_l2c_cb->p_cback)
    {
        APPL_TRACE_ERROR0("bta_jv_l2cap_server_cback cback not present");
        return;
    }
    ...
    if(event == L2C_SOCK_SUCCESS)
    {
        ...
        if (p_l2c_cb_new_listen)
        {
            ...
            APPL_TRACE_DEBUG0("bta_jv_l2cap_server_cback: got the new listen handle");
            failed = FALSE;
        }
        else
        {
            APPL_TRACE_ERROR0("bta_jv_l2cap_server_cback: failed to create new listen socket");
        }

    }
    else if( (event == L2C_SOCK_CONGESTED) || (event == L2C_SOCK_UNCONGESTED))
    {
        APPL_TRACE_DEBUG1("bta_jv_l2cap_server_cback: L2C_SOCK_CONGESTED %d", event);
        p_l2c_cb->cong = (event == L2C_SOCK_CONGESTED) ? TRUE : FALSE;
...
void bta_jv_l2cap_write(tBTA_JV_MSG *p_data)
{
    ...
    if(p_l2c_cb == NULL)
    {
        APPL_TRACE_ERROR0("bta_jv_l2cap_write no p_l2c_cb found");
        return;
    }
    ...
    if (!evt_data.cong &&
            L2C_SOCK_SUCCESS == SOCK_L2C_WriteData (p_l2c_cb->sock_handle, &evt_data.len))
    {
        evt_data.status = BTA_JV_SUCCESS;
    }
...
static tBTA_JV_L2C_CB *bta_jv_add_listen_l2c (tBTA_JV_L2C_CB *p_l2c_cb)
{
    ...
    APPL_TRACE_DEBUG1("bta_jv_add_listen_l2c  sec id in use:%d", get_sec_id_used());
