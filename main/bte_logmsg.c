...
void BTA_setStackLog( const char* log_layer, int log_level)
{
    ...
    while ( 0 != p_f_map->layer_id_start )
    {
        ...
        if( (NULL != p_f_map->p_f) && !strcmp( p_f_map->trc_name, log_layer))
        {
            ...
            DBG_TRACE_DEBUG2("BTA_setStackLog: New trace level set for layer %s is %d", log_layer, new_level);
            layer_found = 1;
            break;
        }
        p_f_map++;
    ...
    if ( layer_found == 0 )
    {
        DBG_TRACE_DEBUG2("BTA_setStackLog: Unable to set Layer %s with level %d. Layer not found", log_layer, log_level);
    }
}
